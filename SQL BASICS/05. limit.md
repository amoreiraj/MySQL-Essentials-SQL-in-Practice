...
The limit restricts how many rows the SQL query returns. The default value is 100; 
It's telling the database to only return the first 100 rows of the query.
...
The limiting functionality is built to prevent you from accidentally returning millions of rows without meaning to (we've all done it). The following limits the output to 110 results.
